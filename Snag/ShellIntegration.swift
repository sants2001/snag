...
private let log = Logger(subsystem: snagSubsystem, category: "ShellIntegration")

let SNAG_CLI_BIN = Bundle.main.sharedSupportPath.map { ($0 + "/SnagCLI").filePath! }
let SNAG_CLI_LINK = (HOME / ".local/bin/snag")
...
class ShellIntegration {
    ...
    /// Whether the `snag` symlink exists and points at this bundle's CLI binary.
    /// Catches stale links (the app was moved or updated to a new path) and broken
    /// links that `SNAG_CLI_LINK.exists` would miss, so the UI offers a repairing
    /// reinstall instead of claiming the CLI is already installed.
    static var isInstalled: Bool {
        guard let cliBin = SNAG_CLI_BIN, cliBin.exists else { return false }
    ...
    static func installCLI() -> String {
        guard let cliBin = SNAG_CLI_BIN, cliBin.exists else {
            return "SnagCLI binary not found in app bundle"
        }

        let fm = FileManager.default
        let linkDir = HOME / ".local/bin"
        let linkPath = SNAG_CLI_LINK
        ...
        do {
            ...
            return "Installed `snag` CLI to \(linkPath.shellString)"
        } catch {
