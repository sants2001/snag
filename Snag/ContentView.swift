...
private let log = Logger(subsystem: snagSubsystem, category: "ContentView")
...
struct ContentView: View {
    ...
    private var resultsList: some View {
            ...
            VStack(spacing: 10) {
                ...
                VStack(spacing: 0) {
                    ZStack(alignment: .topTrailing) {
                        ...
                        .onReceive(NotificationCenter.default.publisher(for: .snagDidCreateFiles)) { notif in
        ...
        .onReceive(NotificationCenter.default.publisher(for: .snagSortByScore)) { _ in
            fuzzy.sortField = .score
            fuzzy.reverseSort = true
            sortOrder = [] // drop the sort indicator from the previously clicked column
        }
        .onReceive(NotificationCenter.default.publisher(for: .snagRequestExcludeSheet)) { notif in
            guard let paths = notif.object as? [FilePath], !paths.isEmpty else { return }
    ...
    private func installContentShortcutMonitor() {
        ...
        contentShortcutMonitor = NSEvent.addLocalMonitorForEvents(matching: [.keyDown, .flagsChanged]) { event in
            ...
            if let pressed = KeyboardShortcuts.Shortcut(event: event),
               let field = SnagShortcuts.sortField(for: pressed)
            {
