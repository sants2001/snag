...
struct ShortcutsSettingsPane: View {
    var body: some View {
        Form {
                ...
                if !items.isEmpty {
                    Section(segment.title) {
                        ForEach(items) { action in
                            LabeledContent {
                                ShortcutRecorder(name: SnagShortcuts.name(for: action.id)) { _ in
                                    validate(action.id)
                                }
                            } label: {
            ...
            Section("Sorting") {
                ForEach(SnagShortcuts.sortShortcuts) { sort in
                    LabeledContent {
            ...
            Section("Stash") {
                ForEach(SnagShortcuts.utilityShortcuts) { utility in
                    LabeledContent {
            ...
            Section {
                Button("Reset all shortcuts to defaults") {
                    KeyboardShortcuts.reset(SnagShortcuts.allNames)
                    conflict = nil
                }
            }
        }
    ...
    private func validate(_ id: ActionID) {
        validate(name: SnagShortcuts.name(for: id), title: ToolbarAction.byID[id]?.title ?? id.rawValue)
    }
    ...
    private func validate(name: KeyboardShortcuts.Name, title: String) {
        guard let new = KeyboardShortcuts.getShortcut(for: name),
              let other = SnagShortcuts.conflictingTitle(with: new, excluding: name)
        else {
            conflict = nil; return
        }
