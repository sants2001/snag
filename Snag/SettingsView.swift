...
private struct GeneralSettingsPane: View {
    ...
    var body: some View {
        Form {
            ...
            // No Privacy section: there is no crash reporter to opt into. Sentry is not
            // linked into this build at all, so an "off" toggle would imply a "on".
        }
        .formStyle(.grouped)
        .scrollContentBackground(.hidden)
    }
        ...
        Form {
            Section("Default Apps") {
                ...
                SettingRow(
                    ...
                    Menu(shelfApp == SNAG_STASH_APP ? "Snag Stash" : (shelfApp.filePath?.stem ?? "None")) {
                        Button("Snag Stash (built-in)") { shelfApp = SNAG_STASH_APP }
...
private struct SearchSettingsPane: View {
    var body: some View {
        Form {
            ...
            Section("Command Line Tool") {
                DescriptiveToggle(
                    title: "Allow the terminal tool to reach Snag",
                    detail: "Off by default. While on, Snag listens on a system-wide port that any program running as you can connect to, and search results include files readable only because Snag has Full Disk Access. Nothing is reachable from another device.",
                    isOn: $enableCLIService
                )
                .onChange(of: enableCLIService) { _, on in
                    if on { FUZZY.startCLIListeners() } else { FUZZY.stopCLIListeners() }
                }

                SettingRow(
                    title: "Command Line Tool",
                    detail: "Installs `snag` to `~/.local/bin/` for searching from the terminal."
                ) {
                ...
                if ShellIntegration.isInstalled {
                    Text("Installed at \(SNAG_CLI_LINK.shellString)")
    ...
    @Default(.enableCLIService) private var enableCLIService
...
private struct SnagAboutView: View {
    var body: some View {
        VStack(spacing: 14) {
            ...
            Text("Every file on your Mac, found as you type. No account, no telemetry, nothing leaves this machine.")
...
private struct AboutSettingsPane: View {
    var body: some View {
        VStack(spacing: 0) {
            // Snag fork: Lowtech's AboutView takes a `LowtechPro?` and so drags the whole
            // licensing module (and with it Paddle) back into the binary. This one is ours,
            // and it carries the GPL-3 attribution the licence requires rather than pointing
            // at upstream's website, contact form and Discord like the original did.
            SnagAboutView()
