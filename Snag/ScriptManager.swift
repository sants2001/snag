...
private let log = Logger(subsystem: snagSubsystem, category: "ScriptManager")
...
let scriptsFolder: FilePath =
    FileManager.default.urls(for: .applicationScriptsDirectory, in: .userDomainMask).first?
        .appendingPathComponent("Snag", isDirectory: true).filePath ?? "~/.local/snag-scripts".filePath!
...
class ScriptManager {
    ...
    func run(script: URL, args: [String]) {
        ...
        lastScript = script
        var env = shellEnv ?? [:]
        env["SNAG_SEVEN_ZIP"] = SEVEN_ZIP.string
        env["SNAG_DUST"] = DUST.string
        env["SNAG_TREE"] = TREE.string
        env["SNAG_TREEDIFF"] = TREEDIFF.string
