...
private let log = Logger(subsystem: snagSubsystem, category: "RightClickMenu")

extension Notification.Name {
    static let snagRequestRename = Notification.Name("snag.requestRename")
    static let snagDidCreateFiles = Notification.Name("snag.didCreateFiles")
    static let snagRequestExcludeSheet = Notification.Name("snag.requestExcludeSheet")
    static let snagSortByScore = Notification.Name("snag.sortByScore")
}
...
struct RightClickMenu: View {
    ...
    var body: some View {
        ...
        Button("Rename\(orderedSelection.count > 1 ? " (batch)..." : "...")") {
            NotificationCenter.default.post(name: .snagRequestRename, object: nil)
        }
        ...
        Button("Exclude from Index...") {
            ...
            NotificationCenter.default.post(name: .snagRequestExcludeSheet, object: paths)
        }
    ...
    private func duplicateSelection() {
        ...
        if !created.isEmpty {
            NotificationCenter.default.post(name: .snagDidCreateFiles, object: created)
        }
    }
        ...
        Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run {
                ...
                if archivePath.exists {
                    NotificationCenter.default.post(name: .snagDidCreateFiles, object: [archivePath])
                }
            }
        }
    }
    ...
    private func exportAs(type: ExportType) {
        ...
        case .csv: "snag-files.csv"
        case .tsv: "snag-files.tsv"
        case .json: "snag-files.json"
        case .plaintext: "snag-files.txt"
        }
