...
private let log = Logger(subsystem: snagSubsystem, category: "FuzzyClient")
...
let indexFolder: FilePath =
    FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first?
        .appendingPathComponent("com.santino.Snag", isDirectory: true).filePath ?? "/tmp/snag-\(NSUserName())".filePath!

let PIDFILE = "/tmp/snag-\(NSUserName().safeFilename).pid".filePath!
