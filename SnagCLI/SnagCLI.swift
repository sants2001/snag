...
// Shared/SnagIPC.swift and are compiled into both this tool and the Snag app.
...
func sendMachPort(data: Data?, sendTimeout: TimeInterval = 2, recvTimeout: TimeInterval = 10) throws -> Data? {
    guard let port = CFMessagePortCreateRemote(nil, SNAG_PORT_ID) else {
        throw NSError(
            domain: "SnagCLI",
            code: 1,
...
struct Explain: ParsableCommand {
    ...
    mutating func run() throws {
        ...
        let request = SnagRequest(command: .explain, paths: resolved)
        guard let data = try sendMachPort(data: JSONEncoder().encode(request)) else {
            fputs("error: no response from Snag app\n", stderr)
            throw ExitCode.failure
        }
        guard let response = try? JSONDecoder().decode(SnagResponse.self, from: data) else {
...
struct Search: ParsableCommand {
    ...
    private func runMachPort() throws {
        ...
        guard let response = try? JSONDecoder().decode(SnagResponse.self, from: responseData) else {
...
struct Reindex: ParsableCommand {
    ...
    mutating func run() throws {
        ...
        if wait, !cancel {
            let statusReq = SnagRequest(command: .status)
            if let statusData = try? sendMachPort(data: JSONEncoder().encode(statusReq), recvTimeout: 5),
               let statusResp = try? JSONDecoder().decode(SnagResponse.self, from: statusData)
            {
        ...
        let command: SnagCommand = cancel ? .cancelIndex : .reindex
        let request = SnagRequest(command: command, rebuild: rebuild, scopes: scopes.isEmpty && volumes.isEmpty ? nil : scopes, paths: volumes.isEmpty ? nil : volumes)
...
struct Status: ParsableCommand {
    ...
    mutating func run() throws {
        let request = SnagRequest(command: .status)
...
struct Recents: ParsableCommand {
    ...
    mutating func run() throws {
        let request = SnagRequest(command: .recents, maxResults: count)
        guard let data = try sendMachPort(data: JSONEncoder().encode(request), recvTimeout: 5) else {
            fputs("error: no response from Snag app\n", stderr)
            throw ExitCode.failure
        }
...
struct Index: ParsableCommand {
    struct Add: ParsableCommand {
        ...
        mutating func run() throws {
            ...
            let request = SnagRequest(command: .indexAdd, scopes: scope.isEmpty ? nil : scope, paths: resolved)
            guard let data = try sendMachPort(data: JSONEncoder().encode(request)) else {
                fputs("error: no response from Snag app\n", stderr)
                throw ExitCode.failure
            }
    ...
    struct Remove: ParsableCommand {
        ...
        mutating func run() throws {
            let resolved = paths.map { ($0 as NSString).expandingTildeInPath }
            let request = SnagRequest(command: .indexRemove, scopes: scope.isEmpty ? nil : scope, paths: resolved)
            guard let data = try sendMachPort(data: JSONEncoder().encode(request)) else {
                fputs("error: no response from Snag app\n", stderr)
                throw ExitCode.failure
            }
    ...
    struct Has: ParsableCommand {
        ...
        mutating func run() throws {
            let resolved = paths.map { ($0 as NSString).expandingTildeInPath }
            let request = SnagRequest(command: .indexHas, scopes: scope.isEmpty ? nil : scope, paths: resolved)
            guard let data = try sendMachPort(data: JSONEncoder().encode(request)) else {
                fputs("error: no response from Snag app\n", stderr)
                throw ExitCode.failure
            }
